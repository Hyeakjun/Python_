...
LANGUAGE_ALL = {
    'zh_CN': {
        ...
        'LANGUAGE': 'zh_CN',
        '选择模型文件': '选择模型文件',
        'DDSP模型（4.0版本及以后可以不需要）:': 'DDSP 模型(4.0版本及以后可以不需要)',
        ...
        '扩散加速': '扩散加速',
        '扩散深度': '扩散深度',
        '扩散模型文件：.pt格式(自动识别同目录下config.yaml)': '扩散模型文件：.pt格式(自动识别同目录下config.yaml)',
    ...
    'en_US': {
        ...
        'DDSP模型（4.0版本及以后可以不需要）:': 'DDSP model (version 4.0 and later may not be required):',
    ...
    'ja_JP': {
        ...
        'LANGUAGE': 'ja_JP',
        '选择模型文件': 'モデルを選択',
        'DDSP模型（4.0版本及以后可以不需要）:': 'DDSP モデル (バージョン 4.0 以降は必要ない場合があります)：',
